/*
** Name:        aegis128l.c
** Purpose:     Implementation of AEGIS-128L
...
static const aegis128l_implementation *implementation_128l = &aegis128l_soft_implementation;
#elif HAS_AEGIS_AES_HARDWARE == AEGIS_AES_HARDWARE_NEON
static const aegis128l_implementation *implementation_128l = &aegis128l_armcrypto_implementation;
#elif HAS_AEGIS_AES_HARDWARE == AEGIS_AES_HARDWARE_NI
static const aegis128l_implementation *implementation_128l = &aegis128l_aesni_implementation;
#elif HAS_AEGIS_AES_HARDWARE == AEGIS_AES_HARDWARE_ALTIVEC
static const aegis128l_implementation *implementation_128l = &aegis128l_altivec_implementation;
#else
                           ...
                           const uint8_t *ad, size_t adlen, const uint8_t *npub, const uint8_t *k)
{
    ...
    return implementation_128l->encrypt_detached(c, mac, maclen, m, mlen, ad, adlen, npub, k);
}
                           ...
                           const uint8_t *k)
{
    ...
    return implementation_128l->decrypt_detached(m, c, clen, mac, maclen, ad, adlen, npub, k);
}
                     ...
                     const uint8_t *k)
{
    ...
    implementation_128l->state_init(st_, ad, adlen, npub, k);
}
...
{
    return implementation_128l->state_encrypt_update(st_, c, clen_max, written, m, mlen);
}
                                       ...
                                       size_t *written, uint8_t *mac, size_t maclen)
{
    ...
    return implementation_128l->state_encrypt_detached_final(st_, c, clen_max, written, mac, maclen);
}
                              ...
                              size_t maclen)
{
    ...
    return implementation_128l->state_encrypt_final(st_, c, clen_max, written, maclen);
}
...
{
    return implementation_128l->state_decrypt_detached_update(st_, m, mlen_max, written, c, clen);
}
...
{
    implementation_128l->stream(out, len, npub, k);
}
...
aegis128l_encrypt_unauthenticated(uint8_t *c, const uint8_t *m, size_t mlen, const uint8_t *npub,
                                  const uint8_t *k)
{
    implementation_128l->encrypt_unauthenticated(c, m, mlen, npub, k);
}
...
{
    return implementation_128l->state_mac_update(st_, m, mlen);
}
...
aegis128l_mac_final(aegis128l_mac_state *st_, uint8_t *mac, size_t maclen)
{
    ...
    return implementation_128l->state_mac_final(st_, mac, maclen);
}
...
aegis128l_mac_verify(aegis128l_mac_state *st_, const uint8_t *mac, size_t maclen)
{
    ...
    switch (maclen) {
    case 16:
        implementation_128l->state_mac_final(st_, expected_mac, maclen);
...
{
    implementation_128l->state_mac_reset(st_);
}
